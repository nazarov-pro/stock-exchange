...
COPY configs/apps/account.yaml /app/config.yaml
COPY configs/db/migrations/account /db-migrations
...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-w -s" -o /app/account github.com/nazarov-pro/stock-exchange/services/account/cmd
